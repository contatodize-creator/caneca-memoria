  ...
  return <main className="homePage"><Nav/><div className="container homeContainer">
    <section className="productHero">
      <div className="homeHeroCopy">
        ...
        <h1>Um QR. Uma experiência que você atualiza quando quiser.</h1>
        <p className="homeLead">Crie um QR para seu produto e conecte vídeos, fotos, áudios e mensagens. Depois de impresso, você pode mudar o conteúdo sem trocar o código.</p>
        <div className="actions"><Link href="/login?next=/criar" className="button">Criar meu QR grátis</Link><a href="#exemplos" className="button secondary">Ver exemplos</a></div>
        <div className="homeBenefits"><span>✓ QR dinâmico</span><span>✓ Sem aplicativo</span><span>✓ Conteúdo editável</span></div>
      </div>
      <div className="homeHeroVisual"><HeroExperience/></div>
    </section>

    <section className="productProof" id="exemplos">
      <div className="homeSectionHeader"><span className="eyebrow">VEJA O PRODUTO</span><h2>O QR está no produto. A experiência aparece no celular.</h2><p className="lead">A DizeCode conecta o que você vende ou presenteia a um conteúdo digital que continua editável.</p></div>
      <div className="qrExampleGrid">{qrExamples.map((item,i)=><article className="qrExampleCard" key={item.title}><div className="qrExampleTop"><div className="fakeQr" aria-label="Exemplo visual de QR"><i/><i/><i/><span/></div><div className="qrArrow">→</div><div className="phonePreview"><div className="phoneNotch"/><div className="phoneIcon">{item.icon}</div><b>{item.title}</b><small>{item.type}</small><div className="phoneLine"/><div className="phoneLine short"/></div></div><h3>{item.title}</h3><p>{item.text}</p><span className="exampleLabel">Exemplo {i+1} • QR demonstrativo</span></article>)}</div>
      <p className="demoNote">Os QRs acima são representações visuais. Na próxima etapa vamos substituir por DizeCodes reais para o visitante escanear e testar.</p>
    </section>

    <section className="productSteps">
      <div className="homeSectionHeader"><span className="eyebrow">DO PAINEL PARA O PRODUTO</span><h2>Você cria. Imprime. E continua no controle.</h2></div>
      <div className="productStepGrid"><div><b>1</b><span>▦</span><h3>Crie seu DizeCode</h3><p>Gere um QR único pelo painel.</p></div><div><b>2</b><span>▶</span><h3>Adicione a experiência</h3><p>Coloque mensagem, foto, vídeo ou áudio.</p></div><div><b>3</b><span>▣</span><h3>Aplique no produto</h3><p>Baixe o QR e use na sua arte, etiqueta ou embalagem.</p></div><div><b>4</b><span>↻</span><h3>Atualize depois</h3><p>Altere o conteúdo sem precisar reimprimir o QR.</p></div></div>
      <div className="centerProductCta"><Link href="/login?next=/criar" className="button">Criar meu primeiro DizeCode</Link></div>
    </section>
