...
const PLAN: Record<string, string> = { pro: "pro", reseller: "reseller", business: "business" };
...
async function supabase(path: string, init: RequestInit = {}) {
  ...
  return fetch(`${url}/rest/v1/${path}`, { ...init, headers: { apikey: key, Authorization: `Bearer ${key}`, "Content-Type": "application/json", Prefer: "resolution=merge-duplicates,return=minimal", ...(init.headers || {}) } });
}
...
async function activate(userId: string, product: string, providerId: string, periodEnd?: string | null) {
  if (product === "experience") {
    ...
    const write = await supabase("account_plans?on_conflict=user_id", { method: "POST", body: JSON.stringify({ user_id: userId, plan: PLAN[product], status: "active", current_period_end: periodEnd || null, updated_at: new Date().toISOString() }) });
    if (!write.ok) throw new Error("Falha ao liberar plano");
  }
  console.info("payment_activated", { userId, product, providerId });
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === "subscription_preapproval" || type === "preapproval") {
      ...
      if (s.status === "authorized" && Math.abs(amount - PRICES[ref.product]) <= 0.001 && String(s.auto_recurring?.currency_id) === "BRL") {
        await activate(ref.userId, ref.product, String(s.id), s.next_payment_date || null);
      } else if (["cancelled", "paused"].includes(String(s.status))) {
        const r = await supabase(`account_plans?user_id=eq.${ref.userId}`, { method: "PATCH", body: JSON.stringify({ status: "canceled", updated_at: new Date().toISOString() }) });
        if (!r.ok) throw new Error("Falha ao cancelar plano");
      }
      return NextResponse.json({ ok: true });
    }
    ...
    console.error("mercadopago_webhook_error", e?.message || e);
